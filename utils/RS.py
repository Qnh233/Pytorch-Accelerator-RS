...
def main():
    ...
    with torch.no_grad():
        ...
        inference_huge_image(opt=opt, img_A_path=opt.A_path, img_B_path=opt.B_path, model=net, eue=eue,
    ...
    for i in range(num_h + 1):
        for j in range(num_w + 1):
            ...
            if len(patches_A) == batch_size:
                process_batch(patches_A, patches_B, coords, model, eue, predict, uncertainty_predict, entropy_predict,
                              count, img_size, st)
                patches_A, patches_B, coords = [], [], []

    # Process remaining patches
    if patches_A:
        process_batch(patches_A, patches_B, coords, model, eue, predict, uncertainty_predict, entropy_predict, count,
                      img_size, st)
...
def process_batch(patches_A, patches_B, coords, model, eue, predict, uncertainty_predict, entropy_predict, count,
                  img_size, st):
    ...
    for idx, (i, j) in enumerate(tqdm(coords)):
        ...
        uncertainty_predict[:, :, i * st: img_size + i * st, j * st: img_size + j * st] += estimated_patches[
            idx].cpu().numpy()
        predict[:, :, i * st: img_size + i * st, j * st: img_size + j * st] += torch.sigmoid(output_patches.detach())[
            idx].cpu().numpy()
        entropy_predict[:, :, i * st: img_size + i * st, j * st: img_size + j * st] += entropy_patches[
            idx].cpu().numpy()
